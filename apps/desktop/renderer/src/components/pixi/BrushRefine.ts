...
export default function BrushRefine(props:{
  ...
  useEffect(()=>{
    ...
    ctx.fillStyle="black"; ctx.fillRect(0,0,imageW,imageH);
    ...
    maskSprite.tint = 0xFFFF00; maskSprite.alpha = 0.45;
    ...
    function onMove(e:any){
      ...
      // 只在 ROI 内编辑
      if(p.x<roi.x1||p.x>roi.x2||p.y<roi.y1||p.y>roi.y2){ last=null; return; }
      const add = state.current.mode===1;
      const r = state.current.radius;
      if(!last){ stamp(p.x,p.y, add); last={x:p.x,y:p.y}; return; }
      // 沿线盖章（半径一半步长）
      ...
      for(let i=0;i<=n;i++){
        const t=i/n; stamp(last.x+dx*t, last.y+dy*t, add);
      }
      last={x:p.x,y:p.y};
    }
    ...
    hit.on("pointerup", ()=>{ state.current.brushing=false; last=null; });
    ...
    function key(ev:KeyboardEvent){
      if(ev.key==="[" ){ state.current.radius=Math.max(1, state.current.radius-1); setHint(`半径=${state.current.radius}`); }
      if(ev.key==="]" ){ state.current.radius=Math.min(128, state.current.radius+1); setHint(`半径=${state.current.radius}`); }
      if(ev.key==="Escape") { onCancel(); }
      if((ev.ctrlKey||ev.metaKey) && (ev.key==="e"||ev.key==="E")) { exportNow(); }
    }
    window.addEventListener("keydown", key);

    return ()=>{ window.removeEventListener("keydown", key); destroy(); };
